...
<script>
	...
	export default {
		...
		methods: {
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						list.forEach(i => {
							...
							params.name = 'file';
							params.uri = i;
							promiseList.push(imgUpload([params]));
						})
						Promise.all(promiseList).then(res=>{
							let urlList = res.map(i=>{
								return i.url;
							})
							this.imageList = this.imageList.concat(urlList);
							this.$emit('submitUrl',this.imageList);
						}).catch((error) => {
							console.log(error)
						})
					},
			...
			// 				let imgPathSize = result,
			// 				this.compressImage(imgPathUrl,size);
			// 			}
			// 		)
			// 	})
			// }
